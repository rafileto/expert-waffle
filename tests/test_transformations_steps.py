...
class TestDataFrameTransformations(unittest.TestCase):
    ...
    def test_rename_and_select_cols(self):
        ...
        df = self.spark.createDataFrame(
            data, ["_c0", "_c2", "_c3", "_c4", "_c5", "_c6", "_c7"]
        )
    ...
    def test_str_to_date(self):
        ...
        self.assertEqual(
            result_df.collect()[0]["trat_data_col"],
    ...
    def test_build_day_of_week_column(self):
        data = [("2024-10-01",)]
        df = self.spark.createDataFrame(data, ["trat_data_horario_requisicao"])
        df = df.withColumn(
            "trat_data_horario_requisicao", F.to_date(df["trat_data_horario_requisicao"])
        )
