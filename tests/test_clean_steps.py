...
class TestSparkDataTransformations(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.spark = SparkSession.builder.master("local").appName("DataTransformationTests").getOrCreate()
    ...
    def test_arrow_datetype_convert(self):
        df = self.spark.createDataFrame([("01/Jan/2024",), ("",), (None,)], ["date_str"])
    ...
    def test_generate_spark_data_profile(self):
        ...
        self.assertAlmostEqual(profile["numeric_col"]["media"], Decimal('2.6667'), places=4)
